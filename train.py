...
def train(
    ...
    #record_result_loss(train_f1s,train_accuracies,dev_f1s,dev_accuracies)
    writer.close()

def record_result_loss(train_f1s,train_accuracies,dev_f1s,dev_accuracies):
    file = open('result/ECMUL_train_rnn.csv','w+',encoding='utf-8')
    ...
    fw = open('result/ECMUL_dev_rnn.csv','w+',encoding='utf-8')
    writer = csv.writer(fw)
    writer.writerow(['index','f1','accuracy'])
    for i in range(len(dev_f1s)):
        dev_f1 = dev_f1s[i]
        dev_acc =dev_accuracies[i]
...
def main():
    ...
    sentences = []
    labels = []
    fr = open('corpora/ECMUL/ecmul.txt','r',encoding='utf-8')
    line = fr.readline()
    while line:
        content = line.split('\t')
        # print(content[0])
