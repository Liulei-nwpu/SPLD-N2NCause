...
'''
def mae(predictions,labels):
    return tf.abs(labels-predictions)
...
def contrastive_lecun(x1, x2, labels, predictions, margin=tf.constant(0.2), distance=euclidean_distance):
    ...
    c_loss2 = mse(predictions,labels)
    ...
    # return 0.1*tf.reduce_mean(tf.reduce_sum(c_loss1)) +   c_loss2
    #return tf.reduce_sum(c_loss1)
    return 0.4 * c_loss1 + 0.6 * c_loss2
